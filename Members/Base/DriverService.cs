...
namespace Members.Base {
  public class DriverService {
    ...
    public static ContentPartFieldDefinition
    GetFieldDef(BuildFieldEditorContext context, bool isAdminTheme) {
      IFieldEditorSettings partSettings = null;
      var oldDef = context.PartFieldDefinition;
      foreach (var typ in ImplementingTypes) {
        context.TypePartDefinition.Settings.TryGetValue(typ.Name,
                                                        out JToken val);
        if (val == null)
          continue;
      ...
      if (partSettings != null) {
        ...
        var newEditor = partSettings.GetFieldDisplayMode(
            context.PartFieldDefinition.Name, textset.Editor, context,
            isAdminTheme);
        ...
        if (textset.Editor != newEditor || textset.DisplayName != newDispName) {
          var newDef = new ContentPartFieldDefinition(
              oldDef.FieldDefinition, oldDef.Name,
