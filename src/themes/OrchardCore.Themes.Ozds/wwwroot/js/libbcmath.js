...
var libbcmath = {
  ...
  bc_num : function() {
    ...
    this.n_len =
        null; /* (int) The number of digits before the decimal point. */
    this.n_scale =
        null; /* (int) The number of digits after the decimal point. */
    ...
    this.toString = function() {
      ...
      r = ((this.n_sign == libbcmath.PLUS) ? '' : this.n_sign) +
          tmp.substr(0, this.n_len);
  ...
  safe_emalloc : function(
      size, len, extra) { return Array((size * len) + extra); },

  /**
   * Create a new number
   */
  bc_init_num : function() { return new libbcmath.bc_new_num(1, 0); },
  ...
  isdigit : function(c) { return (isNaN(parseInt(c, 10)) ? false : true); },
   ...
   * Obviously can't work like c does, so we've added an "offset" param so you
   * could do memcpy(dest+1, src, len) as memcpy(dest, 1, src, len) Also only
   * works on arrays
   */
  ...
  bc_out_of_memory : function() { throw new Error("(BC) Out of memory"); }
};
...
libbcmath.bc_add = function(n1, n2, scale_min) {
  ...
  if (n1.n_sign === n2.n_sign) {
    ...
    cmp_res = libbcmath._bc_do_compare(
        n1, n2, false, false); /* Compare magnitudes. */
    switch (cmp_res) {
    case -1:
    ...
    case 0:
      ...
      res_scale =
          libbcmath.MAX(scale_min, libbcmath.MAX(n1.n_scale, n2.n_scale));
 ...
 */
libbcmath.bc_compare = function(
    n1, n2) { return libbcmath._bc_do_compare(n1, n2, true, false); };
 ...
 */
libbcmath._one_mult =
    function(num, n_ptr, size, digit, result, r_ptr) {
  var carry, value; // int
  ...
  if (digit === 0) {
    ...
    if (digit == 1) {
      libbcmath.memcpy(
          result, r_ptr, num, n_ptr, size); // memcpy (result, num, size);
    } else {
      /*  Initialize */
      ...
      while (size-- > 0) {
        ...
        // result[rptr--] = libbcmath.cint(value % libbcmath.BASE); // @CHECK
        // cint //*rptr-- = value % BASE;
        result[rptr--] =
            value % libbcmath.BASE; // @CHECK cint //*rptr-- = value % BASE;
        // carry = libbcmath.cint(value / libbcmath.BASE);   // @CHECK cint
        // //carry = value / BASE;
        carry = Math.floor(
            value / libbcmath.BASE); // @CHECK cint //carry = value / BASE;
      }
  ...
  if (libbcmath.bc_compare(n1, n2, libbcmath.MAX(n1.n_scale, n2.n_scale)) ===
  0) { quot=libbcmath.bc_new_num(1, scale); quot.n_value[0] = 1; return quot;
  }
  */

  /* Test for divide by 1.  If it is we must truncate. */
  // todo: check where scale > 0 too.. can't see why not (ie bc_is_zero - add
  // bc_is_one function)
  if (n2.n_scale === 0) {
    if (n2.n_len === 1 && n2.n_value[0] === 1) {
      qval = libbcmath.bc_new_num(
          n1.n_len, scale); // qval = bc_new_num (n1->n_len, scale);
      qval.n_sign = (n1.n_sign == n2.n_sign ? libbcmath.PLUS : libbcmath.MINUS);
      libbcmath.memset(qval.n_value, n1.n_len, 0,
          scale); // memset (&qval->n_value[n1->n_len],0,scale);
      libbcmath.memcpy(qval.n_value, 0, n1.n_value, 0,
          n1.n_len + libbcmath.MIN(n1.n_scale,
                         scale)); // memcpy (qval->n_value, n1->n_value,
  ...
  n2ptr = n2.n_len + scale2 -
          1; // n2ptr = (unsigned char *) n2.n_value+n2.n_len+scale2-1;
  ...
  num1 = libbcmath.safe_emalloc(1, n1.n_len + n1.n_scale,
      extra + 2); // num1 = (unsigned char *) safe_emalloc (1,
                  // n1.n_len+n1.n_scale, extra+2);
  if (num1 === null) {
    libbcmath.bc_out_of_memory();
  }
  libbcmath.memset(num1, 0, 0,
      n1.n_len + n1.n_scale + extra +
          2); // memset (num1, 0, n1->n_len+n1->n_scale+extra+2);
  libbcmath.memcpy(num1, 1, n1.n_value, 0,
      n1.n_len +
          n1.n_scale); // memcpy (num1+1, n1.n_value, n1.n_len+n1.n_scale);

  len2 = n2.n_len + scale2; // len2 = n2->n_len + scale2;
  num2 = libbcmath.safe_emalloc(
      1, len2, 1); // num2 = (unsigned char *) safe_emalloc (1, len2, 1);
  if (num2 === null) {
    libbcmath.bc_out_of_memory();
  }
  libbcmath.memcpy(
      num2, 0, n2.n_value, 0, len2); // memcpy (num2, n2.n_value, len2);
  ...
  qval = libbcmath.bc_new_num(
      qdigits - scale, scale); // qval = bc_new_num (qdigits-scale,scale);
  libbcmath.memset(
      qval.n_value, 0, 0, qdigits); // memset (qval->n_value, 0, qdigits);

  /* Allocate storage for the temporary storage mval. */
  mval = libbcmath.safe_emalloc(
      1, len2, 1); // mval = (unsigned char *) safe_emalloc (1, len2, 1);
  if (mval === null) {
    libbcmath.bc_out_of_memory();
  }
  ...
  if (!zero) {
    ...
    // //norm =  10 / ((int)*n2ptr + 1);
    norm = Math.floor(
        10 / (n2.n_value[n2ptr] + 1)); // norm =  10 / ((int)*n2ptr + 1);
    if (norm != 1) {
      libbcmath._one_mult(num1, 0, len1 + scale1 + extra + 1, norm, num1,
          0); // libbcmath._one_mult(num1, len1+scale1+extra+1, norm, num1);
      libbcmath._one_mult(n2.n_value, n2ptr, len2, norm, n2.n_value,
          n2ptr); // libbcmath._one_mult(n2ptr, len2, norm, n2ptr);

      // @CHECK Is the pointer affected by the call? if so, maybe need to adjust
      // points on return?
    }
    ...
    while (qdig <= len1 + scale - len2) {
      ...
      if (n2.n_value[n2ptr] == num1[qdig]) {
        ...
        qguess =
            Math.floor((num1[qdig] * 10 + num1[qdig + 1]) / n2.n_value[n2ptr]);
      }
      /* Test qguess. */

      if (n2.n_value[n2ptr + 1] * qguess >
          (num1[qdig] * 10 + num1[qdig + 1] - n2.n_value[n2ptr] * qguess) * 10 +
              num1[qdig +
                   2]) { // if (n2ptr[1]*qguess > (num1[qdig]*10 + num1[qdig+1]
                         // - *n2ptr*qguess)*10 + num1[qdig+2]) {
        qguess--;
        /* And again. */
        if (n2.n_value[n2ptr + 1] * qguess >
            (num1[qdig] * 10 + num1[qdig + 1] - n2.n_value[n2ptr] * qguess) *
                    10 +
                num1[qdig +
                     2]) { // if (n2ptr[1]*qguess > (num1[qdig]*10 +
                           // num1[qdig+1] - *n2ptr*qguess)*10 + num1[qdig+2])
          qguess--;
        }
      }
      ...
      if (qguess !== 0) {
        ...
        libbcmath._one_mult(n2.n_value, n2ptr, len2, qguess, mval,
            1); //_one_mult (n2ptr, len2, qguess, mval+1); // @CHECK

        ptr1 = qdig + len2; //(unsigned char *) num1+qdig+len2;
        ptr2 = len2;        //(unsigned char *) mval+len2;

        // @CHECK: Does a negative pointer return null?
        //         ptr2 can be < 0 here as ptr1 = len2, thus count < len2+1 will
        //         always fail ?
        for (count = 0; count < len2 + 1; count++) {
          if (ptr2 < 0) {
            // val = libbcmath.cint(num1[ptr1]) - 0 - borrow;    //val = (int)
            // *ptr1 - (int) *ptr2-- - borrow;
            val = num1[ptr1] - 0 -
                  borrow; // val = (int) *ptr1 - (int) *ptr2-- - borrow;
          } else {
            // val = libbcmath.cint(num1[ptr1]) - libbcmath.cint(mval[ptr2--]) -
            // borrow;    //val = (int) *ptr1 - (int) *ptr2-- - borrow;
            val = num1[ptr1] - mval[ptr2--] -
                  borrow; // val = (int) *ptr1 - (int) *ptr2-- - borrow;
          }
          if (val < 0) {
            val += 10;
      ...
      if (borrow == 1) {
        ...
        for (count = 0; count < len2; count++) {
          if (ptr2 < 0) {
            // val = libbcmath.cint(num1[ptr1]) + 0 + carry; //val = (int) *ptr1
            // + (int) *ptr2-- + carry;
            val = num1[ptr1] + 0 +
                  carry; // val = (int) *ptr1 + (int) *ptr2-- + carry;
          } else {
            // val = libbcmath.cint(num1[ptr1]) +
            // libbcmath.cint(n2.n_value[ptr2--]) + carry; //val = (int) *ptr1 +
            // (int) *ptr2-- + carry;
            val = num1[ptr1] + n2.n_value[ptr2--] +
                  carry; // val = (int) *ptr1 + (int) *ptr2-- + carry;
          }
          if (val > 9) {
            val -= 10;
        ...
        if (carry == 1) {
          // num1[ptr1] = libbcmath.cint((num1[ptr1] + 1) % 10);  // *ptr1 =
          // (*ptr1 + 1) % 10; // @CHECK
          num1[ptr1] =
              (num1[ptr1] + 1) % 10; // *ptr1 = (*ptr1 + 1) % 10; // @CHECK
        }
      }
 ...
 * 3. if the sum of the 2 numbers > 9, carry 1 to the next set and subtract 10
...
libbcmath._bc_do_add = function(n1, n2, scale_min) {
  ...
  */
  ...
  // Add the fraction part.  First copy the longer fraction (ie when
  // adding 1.2345 to 1 we know .2345 is correct already) .
  if (n1bytes != n2bytes) {
    if (n1bytes > n2bytes) {
 ...
 * 3. if the sum of the 2 numbers < 0, carry -1 to the next set and add 10 (ie
 18 > carry 1 becomes 8). thus 0.9 + 0.9 = 1.8
 *
 * @param bc_num n1
 * @param bc_num n2
 * @param int scale_min
...
libbcmath.MUL_SMALL_DIGITS = (libbcmath.MUL_BASE_DIGITS /
                              4); //#define MUL_SMALL_DIGITS mul_base_digits/4
...
libbcmath.bc_multiply = function(n1, n2, scale) {
  ...
  prod_scale = libbcmath.MIN(
      full_scale, libbcmath.MAX(scale, libbcmath.MAX(n1.n_scale, n2.n_scale)));
...
libbcmath._bc_simp_mul = function(n1, n1len, n2, n2len, full_scale) {
  ...
  for (indx = 0; indx < prodlen - 1; indx++) {
    n1ptr = n1end -
            libbcmath.MAX(
                0, indx - n2len + 1); //(char *) (n1end - MAX(0, indx-n2len+1));
    n2ptr = n2end - libbcmath.MIN(indx,
                        n2len - 1); //(char *) (n2end - MIN(indx, n2len-1));
    while ((n1ptr >= 0) && (n2ptr <= n2end)) {
      sum += n1.n_value[n1ptr--] *
             n2.n_value[n2ptr++]; // sum += *n1ptr-- * *n2ptr++;
    }
    prod.n_value[pvptr--] =
        Math.floor(sum % libbcmath.BASE);   //*pvptr-- = sum % BASE;
...
libbcmath._bc_shift_addsub = function(accum, val, shift, sub) {
  ...
  if (!(accum.n_len + accum.n_scale >= shift + count)) {
    throw new Error(
        "len + scale < shift + count"); // ?? I think thats what assert does :)
  }

  // Set up pointers and others
  accp = accum.n_len + accum.n_scale - shift -
         1; // (signed char *)(accum->n_value + accum->n_len + accum->n_scale -
            // shift - 1);
  ...
  if (sub) {
    ...
    while (count--) {
      accum.n_value[accp] -=
          val.n_value[valp--] + carry; //*accp -= *valp-- + carry;
...
libbcmath._bc_rec_mul = function(u, ulen, v, vlen, full_scale) {
  ...
  if ((ulen + vlen) < libbcmath.MUL_BASE_DIGITS ||
      ulen < libbcmath.MUL_SMALL_DIGITS || vlen < libbcmath.MUL_SMALL_DIGITS) {
...
libbcmath.bc_sub = function(n1, n2, scale_min) {
  ...
  if (n1.n_sign != n2.n_sign) {
    ...
    case -1:
      ...
      diff.n_sign =
          (n2.n_sign == libbcmath.PLUS ? libbcmath.MINUS : libbcmath.PLUS);
      break;
    case 0:
 ...
 * @param int [scale]               The optional parameter is used to set the
 *     number of digits after the decimal place in the result. You can also set
 *     the global scale for all functions by using bcscale()
 * @return string
 */
function bcadd(left_operand, right_operand, scale) {
  ...
  if (first.n_scale > second.n_scale)
    second.setScale(first.n_scale);
  if (second.n_scale > first.n_scale)
    first.setScale(second.n_scale);
 ...
 *     number of digits after the decimal place in the result. You can also set
 *     the global scale for all functions by using bcscale()
 * @return int                      0: Left/Right are equal, 1 if left > right,
 *     -1 otherwise
 */
function bccomp(left_operand, right_operand, scale) {
  ...
  first = libbcmath.bc_str2num(
      left_operand.toString(), scale); // note bc_ not php_str2num
  second = libbcmath.bc_str2num(
      right_operand.toString(), scale); // note bc_ not php_str2num
...
/**
 * bcround - Returns the rounded value of [val] to the specified [precision]
 * (number of digits after the decimal point). [precision] can also be a
 * negative or zero (default) Note: uses "round up and away from zero" method
 * (ie -1.5 > -2, 1.5 > 2 where .5 always goes to 1 (or 0.5 to -1) etc
 *
 * @param string val            The value to round (accept in virtually any
 *     format)
 * @param int    precision      The optional number of digits to round-to
 * @return string               In exact decimal places of precision (ie
 *     bcround('1.2222', 2) == '1.22' or bcround('1', 4) == '1.0000' )
 */
