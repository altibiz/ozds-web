...
namespace Ozds.Modules.Members.Payments
{
  ...
  public class BankStatPartService : PartService<BankStatPart>
  {
    ...
    public static BsJson ParseStmt(string xmlOrJson)
    {
      ...
      var strDate =
          document.XPathSelectElement("/BkToCstmrStmt/Stmt/FrToDt/FrDtTm")
              ?.Value;
                   ...
                   "/BkToCstmrStmt/Stmt/Ntry"))
      {
        ...
        rrn.Model =
            nTry.XPathSelectElement("NtryDtls/TxDtls/Refs/EndToEndId").Value;
        rrn.Number = nTry.XPathSelectElement(
                             "NtryDtls/TxDtls/RmtInf/Strd/CdtrRefInf/Ref")
                         .Value;

        cPartner.Name =
            nTry.XPathSelectElement("NtryDtls/TxDtls/RltdPties/Dbtr/Nm")
                ?.Value ??
            nTry.XPathSelectElement("NtryDtls/TxDtls/RltdPties/Cdtr/Nm")?.Value;
        cAddress.Street =
            nTry.XPathSelectElement(
                    "NtryDtls/TxDtls/RltdPties/Dbtr/PstlAdr/AdrLine")
                ?.Value ??
            nTry.XPathSelectElement(
                    "NtryDtls/TxDtls/RltdPties/Cdtr/PstlAdr/AdrLine")
                ?.Value;

        rrn.Model =
            document.XPathSelectElement("BkToCstmrStmt/Stmt/Ntry").Value;

        stat.Type =
            nTry.XPathSelectElement("NtryDtls/TxDtls/RmtInf/Strd/AddtlRmtInf")
                .Value;
        stat.PaymentDescription =
            nTry.XPathSelectElement("NtryDtls/TxDtls/RmtInf/Strd/AddtlRmtInf")
                .Value;
        stat.Amount = decimal.Parse(
            nTry.XPathSelectElement("NtryDtls/TxDtls/AmtDtls/TxAmt/Amt").Value,
            CultureInfo.InvariantCulture);
        stat.Type =
            nTry.XPathSelectElement("BkTxCd/Domn/Fmly/Cd").Value.Equals("RCDT")
                ? "Uplata"
                : "Isplata";
        stat.RRN = rrn;
