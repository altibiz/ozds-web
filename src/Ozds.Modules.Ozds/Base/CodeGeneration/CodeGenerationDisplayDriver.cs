...
namespace Ozds.Modules.Ozds
{
      ...
      : ContentTypeDefinitionDisplayDriver
  {
    ...
    public CodeGenerationDisplayDriver(
        IStringLocalizer<CodeGenerationDisplayDriver> stringLocalizer) => T =
        stringLocalizer;
    ...
    private void GenerateCodeForParts(
        StringBuilder codeBuilder, IEnumerable<ContentTypePartDefinition> parts)
    {
    ...
    private void GenerateModelsForParts(
        StringBuilder codeBuilder, IEnumerable<ContentTypePartDefinition> parts)
    {
      var partDefinitions = parts.Where(part => part.PartDefinition.Fields.Any())
      ...
      foreach (var part in partDefinitions)
      {
        ...
        {
          codeBuilder.AppendLine(
              $"    public {field.FieldDefinition.Name} {field.Name} {{ get; set; }}");
        }
    ...
    /// <summary>
    /// Building those parts that have fields separately (fields can't be
    /// configured inline in types).
    /// </summary>
        ...
        StringBuilder codeBuilder, IEnumerable<ContentTypePartDefinition> parts)
    {
      ...
      foreach (var part in partDefinitions)
      {
        codeBuilder.AppendLine();
        codeBuilder.AppendLine(
            $"_contentDefinitionManager.AlterPartDefinition(\"{part.Name}\", part => part");

        var partSettings = part.GetSettings<ContentPartSettings>();
        if (partSettings.Attachable)
          codeBuilder.AppendLine("    .Attachable()");
        if (partSettings.Reusable)
        ...
        {
          codeBuilder.AppendLine(
              $"    .WithField(\"{field.Name}\", field => field");
          codeBuilder.AppendLine(
    ...
    private string ConvertJToken(JToken jToken) => jToken switch
    {
      JValue
          jValue => jValue.Value switch
          {
            bool boolValue => boolValue ? "true" : "false",
            string value => $"\"{value}\"",
            object value => value.ToString()?.Replace(',', '.') ?? "",
            null => "",
          },
      JArray jArray =>
          $"new[] {{ {string.Join(", ", jArray.Select(ConvertJToken))} }}",
      JObject jObject => T
        ...
        StringBuilder codeBuilder, JObject settings, int indentationDepth)
    {
      ...
      {
        var properties = setting.Value.Where(property => property is JProperty)
                             .Cast<JProperty>()
                             .ToArray();
        ...
        codeBuilder.AppendLine(
            $"{indentation}.WithSettings(new {setting.Key}");
        codeBuilder.AppendLine(indentation + "{");

        // This doesn't support multi-level object hierarchies for settings
        // but come on, who uses complex settings objects?
        for (int i = 0; i < properties.Length; i++)
        {
          ...
          codeBuilder.AppendLine(
              $"{indentation}    {property.Name} = {propertyValue},");
        }
    ...
    private static void GenerateCodeForSettings(
        StringBuilder codeBuilder, ContentTypeSettings contentTypeSettings)
    {
      if (contentTypeSettings.Creatable)
        codeBuilder.AppendLine("    .Creatable()");
      if (contentTypeSettings.Listable)
        codeBuilder.AppendLine("    .Listable()");
      if (contentTypeSettings.Draftable)
        codeBuilder.AppendLine("    .Draftable()");
      if (contentTypeSettings.Versionable)
        codeBuilder.AppendLine("    .Versionable()");
      if (contentTypeSettings.Securable)
        codeBuilder.AppendLine("    .Securable()");
      if (!string.IsNullOrEmpty(contentTypeSettings.Stereotype))
      {
        codeBuilder.AppendLine(
            $"    .Stereotype(\"{contentTypeSettings.Stereotype}\")");
      }
    }
