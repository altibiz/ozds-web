...
namespace Ozds.Members.Controllers
{
  [ApiController]
  ...
  public class LegacyAccountController : Controller
  {
    ...
    public async Task<IActionResult> Post(LegacyLoginModel model)
    {
      ...
      if (user == null)
      {
        ...
        var oldLoginItem = await _session
                               .Query<ContentItem, PersonPartIndex>(
                                   x => x.Oib == model.UserName)
                               .FirstOrDefaultAsync();
        ...
        if (oldLoginItem == null ||
            memberItem == null) return Ok(model.UserName);
        ...
        {
          var usr = await _userService.GetUserByUniqueIdAsync(
              mem.User.UserIds[0]);
          if (usr != null)
          {
        ...
        var email = memPerson?.Email?.Text ?? oldPerson?.Email?.Text;

        if (string.IsNullOrWhiteSpace(email)) return ValidationProblem(
            "Član nema upisanu email adresu, molim kontaktirajte udrugu sa svojim OIB-om");
        var newusrname = memPerson?.Oib?.Text ?? oldPerson?.Oib?.Text ??
                         email?.Replace("@", "_");

        var passWord = newusrname + Guid.NewGuid();
        if (!string.IsNullOrEmpty(oldPerson.OldHash) &&
            !string.IsNullOrEmpty(oldPerson.OldSalt))
        {
        ...
        var newuser =
            await _userManager.FindByNameAsync(newusrname) ??
        ...
        memberItem.Apply(mem); _session.Save(memberItem);
